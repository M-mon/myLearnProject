...
虽然使用解释器模式的实例不是很多，但对于满足以上特点，且对运行效率要求不是很高的应用实例，
如果用解释器模式来实现，其效果是非常好的，本文将介绍其工作原理与使用方法。
...
1) 文法
文法是用于描述语言的语法结构的形式规则。
没有规矩不成方圆，例如，有些人认为完美爱情的准则是“相互吸引、感情专一、任何一方都没有恋爱经历”，
虽然最后一条准则较苛刻，但任何事情都要有规则，语言也一样，不管它是机器语言还是自然语言，都有它自己的文法规则。
例如，中文中的“句子”的文法如下。
〈句子〉::=〈主语〉〈谓语〉〈宾语〉
〈主语〉::=〈代词〉|〈名词〉
〈谓语〉::=〈动词〉
...
终结符号就是语言中用到的基本元素,一般不能再被分解，用英语的语法来说，就是一个单词视为一个终结符号，多个单词组合一起，除了特殊意义词组，都视为非终结符号。
简单说，文法是对所有使用元素有明确的定义或者说明,如，中文句子是由主谓宾组成，主语是哪些元素，等等在日常中能明确分辨和定义的内容都是文法。
...
并定义一个方法 freeRide(String info) 调用表达式对象的解释方法来对被分析的字符串进行解释。
...
理解：简单说，就是利用特定关键字或关键符号进行规则书写，输入的内容按照书写好的规则进行解析；
      解析从整个内容开始，根据关键字或关键符合进行分割，一直分割到最小单元，最小单元就交给终结符表达式类处理，否则都交给非终结符表达式类；
      非终结符表达式类除了可以分割内容，还可以进行简单判断。
      解释器模式，接触最多的就是sql执行、XML文档解析，还有java本身实际也是这么一个思路。