...
取号、排队、办理具体业务、对银行工作人员进行评分等，其中取号、排队和对银行工作人员进行评分的业务对每个客户是一样的，可以在父类中实现，
    ...
    (1) 抽象类（Abstract Class）：负责给出一个算法的轮廓和骨架。它由一个模板方法和若干个基本方法构成。这些方法的定义如下。
        ...
        ② 基本方法：是整个算法中的一个步骤，包含以下几种类型。
               ...
               **钩子方法：在抽象类中已经实现，包括用于判断的逻辑方法和需要子类重写的空方法两种。
                ...
                在实际中的应用，比如说有一个接口，这个接口里有7个方法，而你只想用其中一个方法，那么这时，你可以写一个抽象类实现这个接口，
                在这个抽象类里将你要用的那个方法设置为abstract,其它方法进行空实现，然后你再继承这个抽象类，就不需要实现其它不用的方法，这就是钩子方法的作用。
                简单说，钩子方法是一个预留的默认方法，为子类提供了可选项，子类需要时重写该方法即可。


    (2) 具体子类（Concrete Class）：实现抽象类中所定义的抽象方法和钩子方法，它们是一个顶级逻辑的一个组成步骤。

【例1】StudyAbroadProcess.java
用模板方法模式实现出国留学手续设计程序。
...
例子：HookTemplateMethod.java
可以通过在具体子类中重写钩子方法 HookMethod1() 和 HookMethod2() 来改变抽象父类中的运行结果。


建造者模式和模版方法模式很相似，但是模版方法更适用与有特定步骤，需要限制步骤实现的情况。
区别：侧重点不同，实现方式不同。
    建造者模式：
         **侧重于创建类结构相同，但结构的组合内容不同的对象。
         **采用组合的方式实现，内部变化在于传递的不同组合对象。
    模版方法模式：
         **侧重于步骤执行和步骤行为变化。
         **采用继承的方式实现，内部变化在于继承的子类，根据子类的实现而改变父类行为。
